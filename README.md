...
## 7. 기능
+ #### 네이버, 카카오, 구글 간편로그인API 기능
+ #### 공공데이터포털API를 이용한 지역축제, 관광지 추천 기능
+ #### 게시판 보기, 쓰기, 수정 기능 (파일업로드 기능을 포함하고 있으며 해당 게시판에서는 여행후기 상품 후기를 남길 수 있고 사이트 화폐인 코인으로 여행일정 및 예약상품을 거래할 수 있음)
+ #### 여행일정 계획을 도와줄 캘린더, 지도, 주석 기능
+ #### 채팅 기능
+ #### 사이트 화폐 코인 충전 기능
